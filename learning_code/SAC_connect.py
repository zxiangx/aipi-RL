...
class PolicyNetContinuous(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        log_prob = log_prob - torch.log((1 - action.pow(2))/2 + 1e-7)# 这个概率我算的明明是这个结果，
        #但是和代码给出的答案不一样？
        action = action * self.action_bound
        return action, log_prob # 直接在forward函数里面计算好了动作及其概率的对数


class QValueNetContinuous(torch.nn.Module):# 正常Q(s,a)网络
...
class SACContinuous:
    ...
    def take_action(self, state):
        ...
        action = self.actor(state)[0]# 把状态传入actor网络，直接返回了动作和概率，无需多余操作
        return [action.item()]

    def calc_target(self, rewards, next_states, dones):  # 计算目标Q值（照搬复杂公式即可）
    ...
    def update(self, transition_dict):
        ...
        # 更新两个Q网络
        td_target = self.calc_target(rewards, next_states, dones)# 两个Q网络使用同一个TD目标更新
